    ...
    public class PeersPanelControl : MonoBehaviour
    {
        ...
        private void Update ()
        {
            ...
            if (peerConnectionManager)
            {
                ...
                {
                    float volume = 0;
                    if(peerConnection.audioSink is VoipAudioSourceOutput)
                    {
                        volume = (peerConnection.audioSink as VoipAudioSourceOutput).lastFrameStats.volume;
                    }
                    ...
                    if (!isMe)
                    {
                        if (peerConnection.audioSink is VoipAudioSourceOutput)
                        {
                            (peerConnection.audioSink as VoipAudioSourceOutput).unityAudioSource.volume = voipVolumeSlider.value;
                        }
                    }
                    else
                    {
