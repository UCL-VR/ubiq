...
import { Room, RoomServer } from './roomserver';
...
function refresh(iceServer: IceServer, roomServer: RoomServer) {
...
function generateCredentials(secret: any, timeoutSeconds: number) {
    var unixTimestamp = (Date.now()/1000 + timeoutSeconds).toString();
    return {
...
function generateSha1Hmac(secret: any, msg: crypto.BinaryLike | crypto.KeyObject){
    let hmac = crypto.createHmac('sha1',secret).setEncoding('base64');
    hmac.write(msg);
...
function link(room : Room, uri : string, username : string, password : string) {
    ...
    var iceServers = iceServerProperty.servers as IceServer[];
    var serverI = 0;
...
// If changes would be made to room properties, push new room args
function unlink(room : Room, uri : string) {
    ...
    var iceServers : IceServer[] = JSON.parse(args.values[propI]);
